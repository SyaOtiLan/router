...
func ResolvePrechargePolicy(spec *adminmodel.ProviderModelSpecification, globalReserve int64) PrechargePolicyResolution {
	...
	policy, _ := NormalizePrechargePolicy(PrechargePolicy{Type: PrechargeTokenizerEstimate, MinimumReserve: globalReserve}, globalReserve)
...
func NormalizePrechargePolicy(policy PrechargePolicy, globalReserve int64) (PrechargePolicy, error) {
	...
	if policy.MinimumReserve == 0 && globalReserve > 0 {
		policy.MinimumReserve = globalReserve
	}
	return policy, nil
}
