...
3. `RetryTimes > 0`，或错误属于必须离开当前渠道的渠道级硬故障（上游额度不足、上游鉴权失败等）。
4. 当前分组下该模型存在其他候选渠道。
...
flowchart TD
    ...
    B -- 否 --> X[直接返回错误]
    B -- 是 --> C[读取 RetryTimes 与错误分类]
    C --> D{RetryTimes > 0 或渠道级硬故障?}
    D -- 否 --> X
    D -- 是 --> E[进入重试循环并遍历剩余候选]
...
1. 一般请求内切换策略取自运行时配置 `RetryTimes`；上游额度不足、上游鉴权失败等渠道级硬故障即使 `RetryTimes=0` 也会尝试剩余候选。
...
- `429` 和普通 `5xx` 不会触发请求内切换。
- 上游额度不足、上游鉴权失败仍会离开当前渠道并尝试剩余候选。
- 对于其它错误，同一个请求只会打一次上游，失败后直接返回。
...
还要满足错误可重试、未指定渠道；普通错误需要 `RetryTimes > 0`，上游额度不足和鉴权失败属于例外，会直接尝试剩余候选。
